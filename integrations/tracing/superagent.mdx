...
Use `respan-exporter-superagent` to wrap the [Superagent](https://superagent.sh/) Python SDK (`safety-agent`). Each call to `guard`, `redact`, `scan`, or `test` is executed as usual and a log is exported to Respan Traces so you can observe safety checks in your Respan dashboard.
