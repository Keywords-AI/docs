...
Use `respan-exporter-braintrust` to export [Braintrust](https://www.braintrust.dev/) log records (including nested spans) to Respan Traces. You can keep using Braintrust's logger APIs; the exporter installs itself as Braintrust's background logger and forwards records to Respan.
