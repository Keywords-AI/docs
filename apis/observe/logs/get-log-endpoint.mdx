...
Each span in the response includes:
...
For other span types (embedding, workflow, etc.), type-specific fields are extracted based on the `log_type`. See [span types](/documentation/getting-started/observability_data_model#log-types) for details.
</Note>

<Note>
You should first get the span list, and based on the unique_id in the list, retrieve the specific span. Learn more about the [span list endpoint](/apis/observe/logs/logs-list-endpoint).
</Note>
