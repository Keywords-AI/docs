---
title: List logs
api: GET https://api.respan.ai/api/request-logs/
---

The Logs List endpoint allows you to retrieve past logs with specified filters.

<Note>
Use the `GET` method for listing logs. Pass all filters as URL query parameters. Note that `POST /api/request-logs/` is used for [creating logs](/apis/observe/logs/request-logging-endpoint), not listing.
</Note>
...
For other span types (embedding, workflow, etc.), type-specific fields are extracted based on the `log_type`. See [log types](/documentation/getting-started/observability_data_model#log-types) for details.
</Note>
...
For example:
```
"https://api.respan.ai/api/request-logs/?page=1&sort_by=-id&is_test=false&all_envs=false&fetch_filters=false&page_size=1"
```
...
You can filter logs directly via URL query parameters. This is useful for sharing filtered views, bookmarking, or building dynamic filter links.
...
<AccordionGroup>
  <Accordion title="Filter by customer identifier">
    ```bash
    GET /api/request-logs/?customer_identifier=user_123
    ```
  </Accordion>

  <Accordion title="Filter with operator (status code not equal to 200)">
    ```bash
    GET /api/request-logs/?status_code[value]=200&status_code[operator]=not
    ```
  </Accordion>
  ...
  <Accordion title="Filter by custom metadata (auto-detected)">
    ...
    GET /api/request-logs/?event_id=xxx
    ```
  </Accordion>

  <Accordion title="Explicit metadata filter">
    ```bash
    GET /api/request-logs/?metadata__user_type=premium
    ```
  </Accordion>
  ...
  <Accordion title="Multiple filters with OR logic">
    ...
    GET /api/request-logs/?status_code[value]=200&status_code[connector]=OR&priority[value]=low&priority[operator]=not&priority[connector]=OR
    ```
  </Accordion>
  ...
  <Accordion title="Mixed known and unknown fields">
    ...
    GET /api/request-logs/?customer_identifier=abc&event_id=xxx&model=gpt-4
    ```
  </Accordion>
</AccordionGroup>
...
<Accordion title="Example: Combined filters">
  ```bash
  # Filter by customer identifier AND status code
  curl -X GET "https://api.respan.ai/api/request-logs/?customer_identifier=user_123&status_code=200&page_size=10" \
    -H "Authorization: Bearer YOUR_API_KEY"
  ```

  ```bash
  # Using operator syntax for advanced filtering
  curl -X GET "https://api.respan.ai/api/request-logs/?customer_identifier=user_123&cost[value]=0.01&cost[operator]=gte" \
    -H "Authorization: Bearer YOUR_API_KEY"
  ```
</Accordion>
...
- Simple `?field=value` syntax supports only equality (`""`) and not-equal (`"not"`) operators
- For complex operators (`gt`, `gte`, `lt`, `lte`, `contains`, etc.), use the bracket syntax: `?field[value]=X&field[operator]=gt`
...
<AccordionGroup>
  <Accordion title="Filter logs with score greater than 4.0">
    ```bash
    curl -X POST "https://api.respan.ai/api/request-logs/" \
      -H "Authorization: Bearer YOUR_API_KEY" \
...
<ParamField body="operator" type="string" required>
  ...
  <Note>
  For a complete list of filter operators and examples, see the [Filters API Reference](/apis/reference/filters_api_reference).
  </Note>
...
```python Python
url = "https://api.respan.ai/api/request-logs/?page=1&sort_by=-id&is_test=false&all_envs=false&fetch_filters=false&page_size=1"
...
url = f"https://api.respan.ai/api/request-logs/?{urlencode(params)}"
headers = {
    "Authorization": f"Bearer {YOUR_RESPAN_API_KEY}",
}
data = {
...
```
```bash cURL
curl -X POST "https://api.respan.ai/api/request-logs/?page=1&sort_by=-id&is_test=false&all_envs=false&fetch_filters=false&page_size=1" \
...
curl -X GET "https://api.respan.ai/api/request-logs/?customer_identifier=user_123&page_size=10" \
-H "Authorization: Bearer YOUR_RESPAN_API_KEY"
```

```bash cURL (URL filters + POST body)
# Combine URL and POST body filters
curl -X POST "https://api.respan.ai/api/request-logs/?customer_identifier=user_123&event_id=xxx" \
-H "Authorization: Bearer YOUR_RESPAN_API_KEY" \
-H "Content-Type: application/json" \
-d '{
  "filters": {
...
curl -X GET "https://api.respan.ai/api/request-logs/?user_tier=premium&department=sales" \
-H "Authorization: Bearer YOUR_RESPAN_API_KEY"
```
</RequestExample>
<ResponseExample>
...
{
    "count": 1250,
    "next": "https://api.respan.ai/api/request-logs/?page=2",
    "previous": null,
    "results": [
       {
        "unique_id": "xxxxx",
