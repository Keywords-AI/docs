...
<Note>
**Prerequisites**: You must first upload an input file using the [Files API](/apis/develop/openai-batch/files/upload) before creating a batch.
</Note>
...
<ParamField body="input_file_id" type="string" required>
  ID of the uploaded JSONL file containing batch requests. Get this from the [upload file](/apis/develop/openai-batch/files/upload) endpoint.
