...
api: GET https://api.respan.ai/api/v1/batches/{batch_id}/
---
...
**Key fields:**
- `status` - Current batch status (see [create batch](/apis/develop/openai-batch/batches/create) for status values)
- `output_file_id` - ID of the output file when batch completes (use with [retrieve file content](/apis/develop/openai-batch/files/content))
