...
Variables are a core part of prompt management. Use `{{variable_name}}` syntax to add dynamic content to your prompts. Variables let you reuse the same prompt template across different inputs: pass values at runtime from your code, or fill them from testset columns when running experiments.
