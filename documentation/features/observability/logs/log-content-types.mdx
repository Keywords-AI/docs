...
All log types use universal `input` and `output` fields and are sent to the same [logging endpoint](/apis/observe/logs/request-logging-endpoint). For the complete field reference, see [Log fields & parameters](/documentation/features/observability/logs/log-parameters).
...
For more on enabling thinking mode via the gateway, see [Enable Thinking](/documentation/features/llm-gateway/advanced-configuration#enable-thinking).
...
These log types are used by [tracing](/documentation/features/observability/traces/quickstart) to represent spans in a trace hierarchy:
...
For details on how to create and orchestrate these span types, see [Orchestration](/documentation/features/observability/traces/span-types).
