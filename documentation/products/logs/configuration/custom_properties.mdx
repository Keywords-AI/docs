...
Here is an example of how to send 2 custom metadata to Respan in the OpenAI Python SDK.
```python {13-18}
...
Here is an example of how to send 2 custom metadata to Respan in the OpenAI TypeScript SDK. In OpenAI TypeScript SDK, you should add a `// @ts-expect-error` before the metadata field.
```typescript {12-16}
...
<Steps>
  <Step title="Get unique_id of the log">
    You need first to get the `unique_id` of the log you want to update. You can get the `unique_id` from the [Logs list endpoint](/apis/observe/logs/logs-list-endpoint). OR you can get the `unique_id` from [Logs page](https://platform.respan.ai/platform/requests)'s `Log ID` column.
