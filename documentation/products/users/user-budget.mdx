...
<Note>Only Respan users who are using the **LLM proxy** can set budget for their users. See how to [set up LLM proxy](/documentation/getting-started/quickstart/gateway) for more information.</Note>


<Tabs>
<Tab title="OpenAI Python SDK">
Here is an example of how to send a user's data with the parameter `customer_params` to Respan in the OpenAI Python SDK.
```python {16-18}
...
Here is an example of how to send a user's data with the parameter `customer_params` to Respan in the OpenAI TypeScript SDK. In OpenAI TypeScript SDK, you should add a `// @ts-expect-error` before the metadata field.
```typescript {15-17}
...
You can also set the budget to a specific user by calling the [User Update endpoint](/apis/observe/users/user-update-endpoint) or the [User Creation endpoint](/apis/observe/users/user-creation-endpoint) for the new users.