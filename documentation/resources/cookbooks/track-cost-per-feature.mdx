...
4. **Custom views**: Create [saved views](/documentation/features/observability/views) with preset filters for recurring cost analysis
...
When a user exceeds their budget, Respan blocks further requests and returns a `429` status. See [User budgets](/documentation/features/observability/customer-identifier#budgets) for details.

## Next steps

<CardGroup cols={2}>
<Card title="Customer identifier" icon="user" href="/documentation/features/observability/customer-identifier">
  Track users, set budgets and rate limits
</Card>
<Card title="Dashboard" icon="chart-line" href="/documentation/features/observability/metrics">
